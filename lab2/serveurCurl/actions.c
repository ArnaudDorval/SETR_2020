...
int verifierNouvelleConnexion(struct requete reqList[], int maxlen, int socket){
    ...
    int i = nouvelleRequete(reqList, maxlen);

    if (i != -1) {
        //int newFd = accept(reqList[i].fdSocket, (struct sockaddr*)&socket, (socklen_t*)maxlen);
        int newFd = accept(socket, NULL, NULL);
        if (newFd != -1) {
            reqList[i].fdSocket = newFd;
            reqList[i].status = REQ_STATUS_LISTEN;
            return 1;
        }
    }
    return 0; 
            ...
            for(int i = 0; i < maxlen; i++){
                if(reqList[i].status == REQ_STATUS_LISTEN && FD_ISSET(reqList[i].fdSocket, &setSockets)){
                    ...
                    if (pid == -1) {
                        ...
                        // close pipe now that we're done
                        close(pipefd[0]);
                        close(pipefd[1]);
...
int traiterTelechargements(struct requete reqList[], int maxlen){
    ...
    if(nfdsPipes > 0){
        // Au moins un pipe est en attente d'une requête
        ...
        if(p > 0){
            ...
            for(int i = 0; i < maxlen; i++){
                if(reqList[i].status == REQ_STATUS_INPROGRESS && FD_ISSET(reqList[i].fdPipe, &setPipes)){
                    ...
                    // On lit les donnees sur le pipe
                    if(VERBOSE)
                        printf("Lecture de la requete sur le pipe %i\n", reqList[i].fdPipe);
                    // Get required buffer size for receiving the data
                    ...
                    // Now that we know the size, allocate the proper buffer size
                    char* buffer = malloc(sizePayload);
                    unsigned int writtenBytes = 0;

                    while (writtenBytes < sizePayload) {
                        octetsTraites = read(reqList[i].fdPipe, buffer+writtenBytes, sizePayload-writtenBytes);
                        writtenBytes += octetsTraites;
                    }
                    ...
                    // Modify length settings
                    reqList[i].len = sizePayload;

                    // Copy new buffer. 
                    reqList[i].buf = realloc(reqList[i].buf, sizePayload);
                    memcpy(reqList[i].buf, buffer, sizePayload);

                    // Modify status LAST to avoid any errors.
                    reqList[i].status = REQ_STATUS_READYTOSEND;

                    // wait for child process to end
                    waitpid(reqList[i].pid, NULL, 0);

                    // free up buffer now that we're done
                    free(buffer);
                    // close last pipe
